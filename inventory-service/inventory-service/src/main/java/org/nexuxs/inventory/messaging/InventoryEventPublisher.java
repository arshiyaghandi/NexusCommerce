...
public class InventoryEventPublisher {
    ...
    public Mono<Void> publishReservedEvent(InventoryReservedEvent event) {
        return Mono.fromFuture(() -> kafkaTemplate.send(NexusTopics.INVENTORY_RESERVED, String.valueOf(event.orderId()), event))
    ...
    public Mono<Void> publishFailedEvent(InventoryFailedEvent event) {
        return Mono.fromFuture(() -> kafkaTemplate.send(NexusTopics.INVENTORY_FAILED, String.valueOf(event.orderId()), event))
